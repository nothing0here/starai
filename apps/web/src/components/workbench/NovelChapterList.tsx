...
export function NovelChapterList({ chapters, currentChapter, totalChapters, onRewrite, onRequestRevision }: Props) {
  ...
  if (!chapters.length) return (
    <div className="rounded-2xl border border-indigo-200 bg-white/70 p-6 text-gray-500 dark:border-indigo-400/20 dark:bg-white/[0.04] dark:text-gray-400">
      <div className="flex items-center justify-between"><div className="flex items-center gap-3"><Clock size={22} className="animate-spin text-indigo-500 dark:text-indigo-300" /><div><p className="text-sm font-semibold text-gray-900 dark:text-white">AI 编辑部已开始创作</p><p className="mt-1 text-xs">正在准备第 {currentChapter || 1} 章，请稍候...</p></div></div><span className="font-mono text-sm text-indigo-600 dark:text-indigo-300">0 / {totalChapters || 0} 章</span></div>
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-indigo-200 bg-gradient-to-r from-indigo-100/80 to-purple-100/70 p-4 dark:border-indigo-400/20 dark:from-indigo-500/10 dark:to-purple-500/10">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div><div className="text-sm font-semibold text-gray-900 dark:text-white">创作进度</div><div className="mt-1 text-xs text-gray-500 dark:text-gray-400">已完成 {chapters.length} / {total} 章 · {totalWords.toLocaleString()} 字</div></div>
          <div className="flex items-center gap-2"><span className="font-mono text-sm text-indigo-600 dark:text-indigo-300">{progress}%</span><button type="button" onClick={() => openRevision("outline")} disabled={!onRequestRevision} className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-indigo-300 px-3 text-xs text-indigo-700 hover:bg-indigo-100 disabled:opacity-40 dark:border-indigo-400/30 dark:text-indigo-200 dark:hover:bg-indigo-400/10">修改大纲</button><button type="button" onClick={() => downloadText("AI小说工坊-全文.txt", bookText)} className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-gray-200 bg-white/70 px-3 text-xs font-semibold text-gray-700 hover:bg-white dark:border-white/10 dark:bg-white/10 dark:text-white dark:hover:bg-white/15"><Download size={14} />下载全文</button></div>
        </div>
        ...
        {currentChapter && currentChapter > chapters.length && <div className="mt-2 flex items-center gap-2 text-xs text-indigo-600 dark:text-indigo-300"><Clock size={14} className="animate-spin" />正在创作第 {currentChapter} 章，已完成内容可先阅读</div>}
      </div>
      ...
      <div className="grid min-h-[520px] grid-cols-1 overflow-hidden rounded-2xl border border-gray-200 bg-white/70 dark:border-white/10 dark:bg-white/[0.03] lg:grid-cols-[250px_minmax(0,1fr)]">
        <aside className="border-b border-gray-200 bg-gray-50/80 p-3 dark:border-white/10 dark:bg-black/10 lg:border-b-0 lg:border-r">
          <div className="mb-2 px-2 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">章节目录</div>
          <div className="max-h-[420px] space-y-1 overflow-y-auto lg:max-h-[620px]">{chapters.map((chapter) => { const active = chapter.chapter_number === selected?.chapter_number; return <button key={chapter.chapter_number} type="button" onClick={() => setSelectedNumber(chapter.chapter_number)} className={`flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-left transition ${active ? "bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-200" : "text-gray-500 hover:bg-white hover:text-gray-800 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-200"}`}><span className="shrink-0">{chapter.status === "completed" ? <Check size={15} className="text-emerald-500 dark:text-emerald-400" /> : <Clock size={15} className="text-amber-500 dark:text-amber-400" />}</span><span className="min-w-0 flex-1 truncate text-sm">第 {chapter.chapter_number} 章 · {chapter.title}</span></button>; })}</div>
        </aside>
        <article className="min-w-0 bg-white/80 p-5 dark:bg-[#111827]/40 sm:p-8">
          {selected ? <><div className="mb-5 flex flex-wrap items-start justify-between gap-3 border-b border-gray-200 pb-4 dark:border-white/10"><div><div className="text-xs text-indigo-600 dark:text-indigo-300">第 {selected.chapter_number} 章</div><h2 className="mt-1 text-xl font-bold text-gray-900 dark:text-white sm:text-2xl">{selected.title}</h2><div className="mt-2 text-xs text-gray-500">{Number(selected.word_count || 0).toLocaleString()} 字</div></div><div className="flex items-center gap-2"><button type="button" onClick={() => openRevision("chapter")} disabled={!onRequestRevision} className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-indigo-300 px-3 text-xs text-indigo-700 hover:bg-indigo-50 disabled:opacity-40 dark:border-indigo-400/30 dark:text-indigo-200 dark:hover:bg-indigo-400/10">修改本章</button><button type="button" onClick={() => downloadText(`第${selected.chapter_number}章-${selected.title || "正文"}.txt`, contentOf(selected))} className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-3 text-xs text-gray-700 hover:bg-gray-50 dark:border-white/10 dark:bg-white/5 dark:text-gray-200 dark:hover:bg-white/10"><Download size={14} />下载本章</button>{onRewrite && selected.status === "completed" && <button type="button" onClick={() => onRewrite(selected.chapter_number)} className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-indigo-300 px-3 text-xs text-indigo-700 hover:bg-indigo-50 dark:border-indigo-400/30 dark:text-indigo-200 dark:hover:bg-indigo-400/10"><RefreshCw size={14} />重写</button>}</div></div>{selected.summary && <div className="mb-5 rounded-xl bg-gray-50 p-3 text-sm leading-6 text-gray-600 dark:bg-white/[0.04] dark:text-gray-400"><span className="mr-2 text-xs text-gray-500">本章摘要</span>{selected.summary}</div>}<div className="whitespace-pre-wrap text-[15px] leading-8 text-gray-700 dark:text-gray-200">{contentOf(selected)}</div></> : <div className="flex h-full items-center justify-center text-sm text-gray-500">选择章节开始阅读</div>}
        </article>
      </div>

      {revisionOpen && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 p-4" onClick={() => setRevisionOpen(false)}><div className="w-full max-w-xl rounded-2xl border border-gray-200 bg-white p-5 shadow-2xl dark:border-white/10 dark:bg-gray-900" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between"><h3 className="font-semibold text-gray-900 dark:text-white">{revisionType === "chapter" ? `修改第 ${selected?.chapter_number || ""} 章` : "修改故事大纲"}</h3><button type="button" onClick={() => setRevisionOpen(false)} className="text-gray-400 hover:text-gray-900 dark:hover:text-white"><X size={18} /></button></div><p className="mt-2 text-xs leading-5 text-gray-500 dark:text-gray-400">提交后会创建一个新的小说版本，原版本保留在历史记录中。请描述需要调整的内容。</p><textarea autoFocus value={revisionText} onChange={(event) => setRevisionText(event.target.value)} placeholder={revisionType === "chapter" ? "例如：加强冲突，把结尾改成悬念，不改变人物设定" : "例如：第3章提前揭示线索，增加一卷终章，将第5章改为感情转折"} className="mt-4 h-32 w-full resize-none rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm leading-6 text-gray-900 outline-none placeholder:text-gray-400 focus:border-indigo-400 dark:border-white/10 dark:bg-black/20 dark:text-white dark:placeholder:text-gray-500" /><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setRevisionOpen(false)} className="rounded-xl px-4 py-2 text-sm text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-white/5">取消</button><button type="button" disabled={!revisionText.trim()} onClick={() => { if (!revisionText.trim()) return; void onRequestRevision?.({ chapterNumber: revisionType === "chapter" ? selected?.chapter_number : undefined, instruction: revisionText.trim() }); setRevisionOpen(false); }} className="rounded-xl bg-indigo-500 px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">生成新版本</button></div></div></div>}
    </div>
  );
}
