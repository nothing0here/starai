...
const emptyRoute = (defaults: { upstreamModel: string; endpoint: string }): RouteForm => ({
  route_name: "",
  provider: "",
  protocol: "openai",
  ...
  extra_params: {},
  runtime_rule: {},
  cost_rule: { billing_type: "per_token", input_cost_per_m: 0, output_cost_per_m: 0 },
  priority: 100,
  weight: 100,
  timeout_seconds: 120,
  max_retries: 0,
...
function normalizedCostRule(value: Record<string, unknown> | undefined) {
  const rule = { ...(value || {}) };
...
export function ModelRoutesEditor({ modelId, upstreamModel, endpoint, onPrimaryConnectionChange }: { modelId: number; upstreamModel: string; endpoint: string; onPrimaryConnectionChange?: (connection: PrimaryRouteConnection) => void }) {
  ...
  const [form, setForm] = useState<RouteForm>(() => emptyRoute({ upstreamModel, endpoint }));
  ...
  const beginCreate = () => {
    const next = emptyRoute({ upstreamModel, endpoint });
    setEditingId(0);
    setForm(next);
  ...
  const beginEdit = (route: ModelRoute) => {
    ...
    setForm({
      ...
      headers: route.headers || {}, extra_params: route.extra_params || {}, runtime_rule: route.runtime_rule || {}, cost_rule: normalizedCostRule(route.cost_rule),
  ...
  const save = async () => {
    try {
      ...
      const payload = {
        ...
        cost_rule: { ...normalizedCostRule(form.cost_rule), billing_type: billingType },
      };
      if (editingId) {
        await adminApi<ModelRoute>(`/models/${modelId}/routes/${editingId}`, { method: "PATCH", body: JSON.stringify(payload) });
      } else {
        await adminApi<ModelRoute>(`/models/${modelId}/routes`, { method: "POST", body: JSON.stringify(payload) });
      }
  ...
  const billingType = String(form.cost_rule?.billing_type || "per_token");
    ...
    <section className="col-span-2 rounded-2xl border border-blue-100 bg-blue-50/40 p-5" onKeyDown={(event) => { if (event.key === "Enter" && event.target instanceof HTMLInputElement) event.preventDefault(); }}>
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-gray-500">同一平台模型可配置多条线路。优先级数字越小越优先；同优先级按权重分流，故障时自动切换。线路成本只用于利润统计，不改变用户价格。</p>
        </div>
      ...
      {editingId !== null && (
        <div className="mt-4 grid grid-cols-2 gap-3 rounded-xl border border-blue-200 bg-white p-4">
          {((editingId !== 0 && routes[0]?.id === editingId) || routes.length === 0) && <p className="col-span-2 rounded-lg bg-blue-50 px-3 py-2 text-xs text-blue-700">这是主线路。保存后，上游模型名、Endpoint、Base URL、协议及鉴权基础字段会同步到模型接入配置；线路名称、超时、权重和上游成本仍只属于该线路。用户售价不会改变，始终读取模型 price_rule。</p>}
          ...
          <p className="mb-3 text-[11px] text-gray-500">这里填写折算后的平台算力成本，用于和用户实际扣费计算毛利；不会向用户展示，也不会改变模型售价。</p>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
              <label className="text-xs text-gray-600">计费方式<select value={billingType} onChange={(e) => setForm({ ...form, cost_rule: { billing_type: e.target.value } })} className="mt-1 w-full rounded-lg border bg-white p-2 text-sm"><option value="per_token">按 Token</option><option value="per_request">按次</option><option value="per_image">按图片</option><option value="per_second">按秒</option></select></label>
