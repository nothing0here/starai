-- Backfill structured assets and storyboards for comic runs that were created
-- before workbench runs automatically received a comic project.
WITH comic_runs AS (
  SELECT
...
asset_rows AS (
  ...
  CROSS JOIN LATERAL (
    ...
    FROM jsonb_array_elements(COALESCE(run.outputs->'comic_plan'->'characters', '[]'::jsonb))
      WITH ORDINALITY AS value(item, ordinality)
    UNION ALL
    SELECT 'prop'::text, item, ordinality
    FROM jsonb_array_elements(COALESCE(run.outputs->'comic_plan'->'props', '[]'::jsonb))
      WITH ORDINALITY AS value(item, ordinality)
    UNION ALL
    SELECT 'location'::text, item, ordinality
    FROM jsonb_array_elements(COALESCE(run.outputs->'comic_plan'->'locations', '[]'::jsonb))
      WITH ORDINALITY AS value(item, ordinality)
  ) source
)
...
FROM comic_runs run
CROSS JOIN LATERAL
  jsonb_array_elements(COALESCE(run.outputs->'comic_plan'->'storyboards', '[]'::jsonb))
